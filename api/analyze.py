"""
/api/analyze — Excel analysis + AI prompt evolution
Vercel auto-maps this file to POST /api/analyze
"""
...
def generate_prompt_deltas(analysis):
    ...
    fixes = {
        'Consent Taken Met':     ('AI rejects passive Hindi consent (Okay/Haan ji/Theek hai)',
                                  '3-Tier: Tier1 explicit, Tier2 contextual after full pitch, Tier3 refusal only'),
        'Charges Explained Met': ('AI penalizes delivery speed not factual accuracy',
                                  'Content-based — pass if ₹699+GST stated correctly, pace irrelevant'),
        'Pitch Pace':            ('AI threshold stricter than human tolerance',
                                  'Only fail if customer explicitly asks to repeat/slow down'),
        'Benefits Explained Met':('AI requires too many benefits',
    ...
    for param, info in sorted(pf.items(), key=lambda x: x[1]['pct'], reverse=True):
        ...
        rc, fix  = fixes.get(param, (f'AI too strict on {param}', f'Align {param} with human verifier'))
...
def evolve_prompt_vertex(analysis, deltas, current_prompt):
    s    = analysis['summary']
    meta = f"""You are an expert prompt engineer for ICICI Bank Debit Card upgrade compliance auditing.
...
CONSENT PATTERNS: {json.dumps(analysis['false_rework']['patterns']['consent'])}
CHARGES PATTERNS: {json.dumps(analysis['false_rework']['patterns']['charges'])}

CURRENT PROMPT:
{current_prompt[:2000] or '(none provided)'}

TASK: Write a production-ready compliance audit prompt that:
1. 3-Tier Hindi consent (Tier1: explicit "haan", Tier2: contextual after full pitch, Tier3: refusal only)
2. Content-based charges (pass if ₹699+GST stated — pace irrelevant)
3. Fuzzy card variant matching for Coral Debit Card variants
4. Fail Pitch Pace ONLY if customer explicitly requests clarification
5. All thresholds aligned to the human verifier data above
